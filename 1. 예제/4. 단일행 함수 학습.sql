...
SELECT
    ...
    SUBSTR('SQL Developer', 2, 5),  -- 문자열 자름 1번부터 3개자름(첫글자가 1번 ~까지)
    LENGTH('HELLO WORLD'),          -- 문자열의 길이
    TRIM('    HI    ') AS TRIM      -- 좌우 공백 제거
FROM dual;
...
SELECT
    ...
    TO_CHAR(SYSDATE,'YY/MM/DD HH24:MI:SS') AS "날짜와시간",
    TO_CHAR(SYSDATE,'YY/MM/DD AM HH12:MI:SS') AS "날짜와시간2"
FROM dual;
